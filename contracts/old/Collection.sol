...
contract Collection is ERC721URIStorage, AccessControl {
    ...
    event Minted(uint256 _tokenId, string _link, address _to);
    ...
    function mint(string memory _link, address _to) public onlyMinters {
        globalCounter.increment();
        _safeMint(_to, globalCounter.current());
        _setTokenURI(globalCounter.current(), _link);
        emit Minted(globalCounter.current(), _link, _to);
    }

    function mintCollection(string[] memory _array, address _to)
        public
        onlyMinters
    {
        for (uint256 i = 0; i < _array.length; i++) {
            globalCounter.increment();
            _safeMint(_to, globalCounter.current());
            _setTokenURI(globalCounter.current(), _array[i]);
        }
    }
